...
`tidyr` y `dplyr` son paquetes que facilitan la manipulación de los datasets y son parte de la colección de paquetes de `tidyverse`

```{r}
library(tidyverse)
```

Ambos paquetes hacen uso de las facilidades del operador `%>%` (pipe, tubo en español) lo que facilita su interpretación lógica: utiliza el resultado de su lado izquierdo como primer argumento de la función del lado derecho (asemejandose a una receta de torta...)
